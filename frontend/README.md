...
- **AI calls are opt-in and cached.** `generatePlan()` (`src/api/plans.ts`) is only called from an explicit button press, never automatically. The plan-refresh cadence preference (weekly/biweekly/monthly) only ever surfaces a reminder banner on the Exercise groups screen (`app/plan/index.tsx`, `src/lib/planRefresh.ts`) — it never triggers a call on its own.
